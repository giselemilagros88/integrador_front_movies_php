...
document.addEventListener('DOMContentLoaded', async () => {
   ...
   const response = await fetch('http://localhost/comision_24143/api_simple/pelicula.php', options);
    ...
    document.querySelectorAll('.btnModificar').forEach(button => {
        ...
        button.addEventListener('click', async (event) => {
            ...
            try {
                const response = await fetch(`http://localhost/comision_24143/api_simple/pelicula.php?id=${peliculaId}`);
                if (!response.ok) {
